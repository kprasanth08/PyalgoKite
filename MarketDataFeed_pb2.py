...
# Wrap the AddSerializedFile call in a try-except to handle duplicate registration
try:
    ...
    # If it's already registered, try to find it in the pool
    if "duplicate file name" in str(e):
        print("MarketDataFeed.proto descriptor already registered, reusing")
        try:
            # Try to find the file by name, which is the correct method for this protobuf version
            DESCRIPTOR = _descriptor_pool.Default().FindFileByName("MarketDataFeed.proto")
        except Exception as find_error:
            print(f"Error finding existing descriptor: {find_error}")
            raise e
    else:
        raise
